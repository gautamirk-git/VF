"""
Builds the POC3 page — proving the engine scales to breadth and can rank.

POC1 proved one rule on one product/region. POC2 proved three rules with
zero cross-triggering. POC3 runs all three rules across 7 product/region
combinations in one pass, ranks every triggered situation by confidence,
and shows only the top 5 — with the promo-driven "fake" Opportunity
correctly discounted and cut. Every number on the page comes out of
ranked_insights.json, nothing hand-typed.
"""
import json

RANKED = json.load(open("ranked_insights.json"))
TOP5 = RANKED["top5"]
CUT = RANKED["cut"]
...
cut_rows = "".join(
    f'''<tr>
      <td><span class="badge sm" style="background:{BADGE_BG[c["situation_type"]]};color:{CARD_COLOR[c["situation_type"]]};">{c["situation_type"]}</span></td>
      <td>{c["product"]}</td><td>{c["region"]}</td><td>{c["confidence"]}%</td>
      <td>{"Promo-influenced — confidence discounted below the cutoff" if c.get("promo_influenced") else "Lower confidence than the top 5"}</td>
    </tr>''' for c in CUT
)
cut_section = ""
if CUT:
    ...
    <section class="cutbox">
      ...
      <p class="sub2">{len(CUT)} more situation{"s" if len(CUT) != 1 else ""} triggered this run but ranked below the top {RANKED["top_n"]} by confidence.</p>
      <div class="table-wrap"><table>
        <thead><tr><th>Type</th><th>Product</th><th>Region</th><th>Confidence</th><th>Why it's cut</th></tr></thead>
        <tbody>{cut_rows}</tbody>
      </table></div>
    </section>
    '''

HTML = f"""<title>Top 5 DotIQ Situations</title>
<style>
...
<div class="page">
  <header>
    <div class="brandrow">
      ...
      <span class="kicker">DotIQ · POC3</span>
    </div>
    <h1>Top 5 Situations, Ranked</h1>
    <p class="dek">POC2 proved three rules on one product/region at a time. POC3 runs the same rules across 3 products × 3 regions in one pass, ranks every situation that fires by confidence, and shows the top {RANKED["top_n"]} — including catching a promo-driven spike that would otherwise look like clean organic demand.</p>
    <div class="proof">✓ 7 product/region combos checked · {RANKED["total_triggered"]} situations triggered · top {RANKED["top_n"]} ranked by confidence</div>
  </header>
  ...
  <footer>
    POC3 — synthetic data, deterministic rules only, no LLM yet (that's POC4). 3 products (Vans Knu Skool, Vans Old Skool, Vans Sk8-Hi) × 3 regions (West Coast, Northeast, Midwest), 7 of the 9 combinations populated. Each product/region is its own independent synthetic window.
  </footer>
</div>
...
with open("poc3_screen.html", "w") as f:
    f.write(HTML)
print("Wrote poc3_screen.html")
